...
function groupByDiscount(data:any) {
  const summary:any = {};

  data.forEach(({ discountName, itemSold, sales }: any) => {
    if (!summary[discountName]) {
      summary[discountName] = { items: 0, sales: 0 };
    }
    summary[discountName].items += itemSold;
    summary[discountName].sales += sales;
  });
...
const DiscountTable = ({ data }:any) => {
  const rows = groupByDiscount(data);
      ...
      <table className="min-w-full">
        <thead>
          <tr className="text-left text-sm font-medium border-b-2 border-gray-300">
            <th className="p-2 border-e-2">Name</th>
            <th className="p-2 border-e-2 text-end">Discounts Applied</th>
            <th className="p-2 border-e-2 text-end">Amount Discounted (฿)</th>
          </tr>
        </thead>
...
const normalizeLineChartData = (rawData:any) => {
  const allServices = Array.from(new Set(rawData.map((d:any) => d.discountName)));

  const groupedByDate: any = {};
  rawData.forEach(({ name, discountName, sales }:any) => {
    if (!groupedByDate[name]) groupedByDate[name] = { name };
    groupedByDate[name][discountName] = sales;
  });
  ...
  return (
    <div>
      ...
      <section className="p-2 my-5">
        <div className="text-xl font-bold">Top 5 Discounts: Amount Discounted</div>
...
export default function Discounts() {
  ...
  const getDashboardData = async (branchId: any = null, startDate:any, endDate: any) => {
    try {
      ...
      const services = await fetch(`${DASHBOARD_API_URL}/discounts${query}`);
  ...
  return (
    <div style={{padding: "20px 40px 20px 30px"}}>
      ...
      {dashboardData?.revenueBarData?.length && <DiscountTable data={dashboardData?.revenueBarData || []} />}
