...
function groupByCategory(data:any) {
  ...
  data.forEach(({ categoryName, itemSold, sales }: any) => {
    if (!summary[categoryName]) {
      summary[categoryName] = { items: 0, sales: 0 };
    }
    summary[categoryName].items += itemSold;
    summary[categoryName].sales += sales;
  });
      ...
      <table className="min-w-full">
        <thead>
          <tr className="text-left text-sm font-medium border-b-2 border-gray-300">
            <th className="p-2 border-e-2">Category</th>
...
const normalizeLineChartData = (rawData:any) => {
  const allServices = Array.from(new Set(rawData.map((d:any) => d.categoryName)));

  const groupedByDate: any = {};
  rawData.forEach(({ name, categoryName, sales }:any) => {
    if (!groupedByDate[name]) groupedByDate[name] = { name };
    groupedByDate[name][categoryName] = sales;
  });
...
export default function CategorySales() {
  ...
  const getDashboardData = async (branchId: any = null, startDate:any, endDate: any) => {
    try {
      ...
      const services = await fetch(`${DASHBOARD_API_URL}/category-sales${query}`);
  ...
  return (
    <div style={{padding: "20px 40px 20px 30px"}}>
      ...
      {dashboardData?.revenueBarData?.length && <CategoryTable data={dashboardData?.revenueBarData || []} />}
